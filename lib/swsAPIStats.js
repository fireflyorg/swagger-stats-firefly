...
swsAPIStats.prototype.countResponse = function (res) {
    ...
    const labels = [req.method,req.sws.api_path,res.statusCode,req.query.symbol||req.body.symbol||''];
    this.promClientMetrics.api_request_total.labels(...labels).inc();

    this.promClientMetrics.api_request_duration_milliseconds.labels(...labels).observe(req.sws.duration);
    this.promClientMetrics.api_request_size_bytes.labels(...labels).observe(req.sws.req_clength);
    this.promClientMetrics.api_response_size_bytes.labels(...labels).observe(req.sws.res_clength);
